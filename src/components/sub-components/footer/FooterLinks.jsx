...
const FooterLinks = ({ heading = "Enter Heading", links }) => {
  ...
  return (
    <details open={isLargeSize} className="w-full lg:border-none md:border-none border-b smd:border-b pb pt-2 font-semibold">
      <summary className="list-none lg:text-xl md:text-lg text-dealbondhu font-sans lg:pointer-events-none md:pointer-events-none text-start md:text-start lg:text-start lg:pr-15 uppercase">
        {heading}
      </summary>
      <ul className="mt-3 ml-2">
        {
            links?.map((link,index)=><li className="text-base font-normal my-2 cursor-pointer hover:underline text-black font-sans" key={index}>{link}</li>)
        }
      </ul>
    </details>
  );
